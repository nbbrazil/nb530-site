...
export default async function handler(req, res) {
  ...
  if (response.ok) {
                // O codigo PIX esta em data.data (campo interno da MagicPay)
              const inner = data.data || {};
                const pixCode = inner.emvqrcps || inner.qrcode || inner.code || inner.emv || inner.brCode || data.pixCode || '';
                const pixQrUrl = inner.qrcodeUrl || inner.qr_url || inner.qrUrl || data.pixQrUrl || '';
                console.log('PIX CODE:', pixCode.substring(0, 50));
                return res.status(200).json({ success: true, orderId, pixCode, pixQrUrl, status: data.status || '' });
  }
